...
int main(int argc, char *argv[])
{
    if (argc != 4)
    {
        printf("Usage: please provide three inputs: a serialized matrix, a "
    ...
    for (int i = 0; i < n && token; i++)
    {
        for (int j = 0; j < n && token; j++)
        {
            ...
            if (weight < 0)
            {
                printf("Usage: please provide three inputs: a serialized "
                       "matrix, a source node and a destination node\n");
                return 1;
            }
            graph[i][j] = weight;
            token = strtok(NULL, ", ");
