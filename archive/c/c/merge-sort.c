...
void merge_sort(long *arr_sort, int l, int r)
{
    ...
    if (l < r)
    {
        ...
        merge_sort(
            arr_sort, l,
            m); // recursive call for first half of subarray arr_sort[l...m]
        merge_sort(
            arr_sort, m + 1,
            r); // recursive call for second half of subarray arr_sort[m+1....r]

        merge_array(arr_sort, l, m, m + 1,
                    r); // merging the two halves into arr_sort[l....r]
    }
}

void merge_array(long *arr_sort, int a, int b, int c, int d)
{
    long t[50]; // Temporary array t created for temporary storage of sorted
                // subarray[l...r] of arr_sort
    ...
    while (i <= b && j <= d)
        if (arr_sort[i] < arr_sort[j])
            ...
            t[k++] = arr_sort[j++]; // Comparing both indices of first half and
                                    // second half and adding smaller one to
                                    // temporary array
