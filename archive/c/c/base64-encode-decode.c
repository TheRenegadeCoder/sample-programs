...
static unsigned char *base64_decode(const unsigned char *input, size_t length,
                                    size_t *output_length);
...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(mode, "encode") == 0)
    {
        ...
        unsigned char *data = base64_encode((const unsigned char *)input_text,
                                            input_length, NULL);
        if (!data)
            show_usage();
    ...
    else if (strcmp(mode, "decode") == 0)
    {
        ...
        unsigned char *data = base64_decode((const unsigned char *)input_text,
                                    ...
                                    size_t *output_length)
{
    ...
    while (input_index < length)
    {
        ...
        data[encoded_index++] =
            (remaining > 1) ? base64_alphabet[(buffer >> 6) & 0x3F] : '=';
        data[encoded_index++] =
            (remaining > 2) ? base64_alphabet[buffer & 0x3F] : '=';
    }
