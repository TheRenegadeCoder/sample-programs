...
This function is used to sort two list together (`xs` and `ys`). There are a different conditions
...
of the first list we convert it into a list `[xs[0]]` because we want to concatenate it with a
...
variable but cannot edit them, in the example above it's the `xs` variable our
unsorted list.
...
So in this example we execute the above lines first you can ignore the `sort()` function until
it's called within `merge_sort()`. `split_xs = [[x] for x in xs]` splits our lists into separate lists.
It takes each element with in `xs` and turns it into a list `[x]`. So `split_xs` now a list of lists.
...
empty the length is less than 0 `len(xs) <= 0`, if so we return an empty list `return []`.
