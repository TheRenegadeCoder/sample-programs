...
We return to the Swift programming language with that old chestnut, Fizz Buzz. [According to urban legend](http://wiki.c2.com/?FizzBuzzTest), Fizz Buzz is frequently used on job interviews to test if the candidate can code (I've never been so lucky).
