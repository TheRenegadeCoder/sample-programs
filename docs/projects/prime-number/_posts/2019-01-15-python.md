...
In this article, we'll be tackling checking if a number is a prime in Python.
...
function and passes user input to it. In this case the user input would be a natural number
greater than 1.
...
Finally we wrap this entire block in a `try ... except`, we catch two exceptions `IndexError`
will be thrown is `args` isn't a list and we try to access `args[0]`. `ValueError` will be thrown
if you try to convert a non-integer string into an integer for example if args[0] was `"a"` ->
`int("a")`. If any exceptions are raised then we call the `exit_with_error()` function.
...
Now onto the main part of the program, this is the function that actually checks if the integer is
a prime number.
...
`https://www.pexels.com/photo/grey-3-chain-1061140/`
